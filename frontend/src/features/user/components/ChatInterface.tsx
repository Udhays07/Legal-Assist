...
export function ChatInterface({ userId }: ChatInterfaceProps) {
    ...
    return (
        <div className="flex flex-col h-full overflow-hidden bg-background relative selection:bg-primary/20">
            ...
            <div className="flex h-12 shrink-0 items-center border-b px-4 justify-between bg-background/60 backdrop-blur-xl z-20 w-full sticky top-0 border-border/50 shadow-sm">
                <div className="flex items-center gap-2">
                    ...
                    <div className="flex items-center gap-2 ml-2 px-2 py-1 rounded-lg hover:bg-muted/50 transition-colors group">
                        ...
                        <Select value={selectedCategoryId || "all"} onValueChange={(val) => setSelectedCategoryId(val === "all" ? "" : val)}>
                            ...
                            <SelectContent position="popper" className="rounded-2xl border-border/50 shadow-2xl">
                ...
                <div className="flex items-center gap-2">
                    <Button
                        ...
                        className="text-muted-foreground hover:text-foreground text-xs font-bold uppercase tracking-wider h-8"
                    >
                        Logout
                    </Button>
                    <ThemeToggle />
            ...
            <ScrollArea className="flex-1 w-full min-h-0 relative z-10 transition-all duration-300">
                <div className="flex flex-col min-h-full pb-32 max-w-2xl mx-auto pt-4">
                    {messages.length === 0 ? (
                        <div className="flex-1 flex flex-col items-center justify-center text-muted-foreground p-10 text-center gap-6 mt-16 animate-in fade-in zoom-in-95 duration-700">
                            <div className="h-20 w-20 bg-gradient-to-br from-primary/20 to-accent/20 rounded-[1.5rem] flex items-center justify-center shadow-xl border border-white/20 dark:border-white/5 relative group">
                                <div className="absolute inset-0 bg-primary/20 rounded-[1.5rem] blur-xl opacity-0 group-hover:opacity-100 transition-opacity"></div>
                                <Sparkles className="h-10 w-10 text-primary relative z-10" />
                            </div>
                            <div className="max-w-md text-center">
                                <h2 className="text-2xl font-extrabold text-foreground mb-2 tracking-tight font-headline">How can I help you today?</h2>
                                <p className="text-muted-foreground leading-relaxed text-sm font-medium opacity-80 decoration-primary/30">
                        ...
                        <div className="flex flex-col w-full space-y-2">
                            ...
                            {isLoading && (
                                <div className="w-full flex justify-start px-4 py-6 animate-in fade-in slide-in-from-left-2 duration-500">
                                    <div className="flex max-w-2xl gap-3 md:gap-4 items-center bg-card/60 backdrop-blur-md rounded-xl px-4 py-2.5 border border-border/50 shadow-lg relative overflow-hidden group">
